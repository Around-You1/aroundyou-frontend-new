...
type AttractionDetail = { id: string; name: string; description?: string; imageUrl?: string; category?: string; distance?: number };
...
export default function AttractionDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/guest/attractions/${id}?accommodationId=${encodeURIComponent(accommodationId)}`);
