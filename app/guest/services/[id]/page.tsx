...
type ServiceDetail = { id: string; name: string; description?: string; imageUrl?: string; category?: string };
...
export default function ServiceDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/guest/services/${id}?accommodationId=${encodeURIComponent(accommodationId)}`);
